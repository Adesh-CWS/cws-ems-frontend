...
const MangerTaskTMS = ({ role }) => {
  ...
  const fetchTaskComments = async (taskId) => {
    ...
    try {
      const response = await axios.get(`http://localhost:8000/task/${taskId}/comments`);
  ...
  const fetchTasks = async () => {
    try {
      ...
      const res = await axios.get(`http://localhost:8000/manager/emp/task/${managerId}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const taskTypeRes = await axios.get(`http://localhost:8000/api/task-types/unique-names`);
        setTaskType(taskTypeRes.data.taskTypes || []);
        
        const projectRes = await axios.get(`http://localhost:8000/api/projects/unique-names/${user._id}`);
